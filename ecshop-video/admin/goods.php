...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_POST['goods_img_url'] != $_LANG['lab_picture_url'] && $_POST['goods_img_url'] != 'http://') && $is_url_goods_img = 1))
    {
        ...
        {
            /* 删除原来的图片文件 */
//            新加了一个goods_thumb_middle字段
            $sql = "SELECT goods_thumb, goods_thumb_middle,goods_img, original_img " .
            ...
            if ($row['goods_thumb_middle'] != '' && is_file('../' . $row['goods_thumb_middle']))
            {
                @unlink('../' . $row['goods_thumb_middle']);
            }
    ...
    $first_start_date = ($is_first && !empty($_POST['first_start_date'])) ? local_strtotime($_POST['first_start_date']) : 0;
    $first_end_date = ($is_first && !empty($_POST['first_end_date'])) ? local_strtotime($_POST['first_end_date']) : 0;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "promote_start_date, promote_end_date, is_first, first_start_date, first_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    "'$promote_start_date', '$promote_end_date','$is_first','$first_start_date','$first_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
    ...
    if (isset($img))
    {
        ...
        //这里加了一个thumb_middle_url字段
        $sql = "INSERT INTO " . $ecs->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url,thumb_middle_url ,img_original) " .
                "VALUES ('$goods_id', '$gallery_img', '', '$gallery_thumb', '$goods_img','$img')";
        $db->query($sql);
    }
